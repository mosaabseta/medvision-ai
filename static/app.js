...
async function refreshTimeline() {
  try {
    ...
    const recentFindings = data.timeline.slice(-8);
    
    if (recentFindings.length === 0) {
      list.innerHTML = '<li style="opacity: 0.5;">No findings yet...</li>';
      return;
    }

    recentFindings.forEach(entry => {
      const li = document.createElement("li");
      li.innerHTML = `<strong>[${entry.time}]</strong><br>${entry.finding.substring(0, 120)}...`;
      list.appendChild(li);
    });
  } catch (error) {
    // Silently fail - timeline may not be available yet
    const list = document.getElementById("timelineList");
    if (list && list.children.length === 0) {
      list.innerHTML = '<li style="opacity: 0.5;">Waiting for analysis...</li>';
    }
  }
}
